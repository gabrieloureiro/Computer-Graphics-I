...
def get_scale(e):
    ...
    if len(e) != 4:
        raise Exception("Must be 4 components")

    # Inicializando com identidade:
    scale = np.identity(4)
...
def get_rotation(theta=None, axis=None, sin=None, cos=None):
    # If axis == x, y or z:
    ...
    if axis == 'x':
        ...
        raise Exception("Must be 'x', 'y', 'z' or None")

    return rotation


def get_arbitrary_rotation(p1, p2, theta):
    """
...
def get_quaternion(p1, p2, theta):
    """
...
def get_shear(axis, direction, alpha):
